...
CruZeta has been subjected to a formal third-party security review. For security
announcements, audit results and other general security information, see
https://cruzeta.org/support/security.html
...
This implementation of CruZeta is not resistant to side-channel attacks. You
should assume (even unprivileged) users who are running on the hardware, or who
are physically near the hardware, that your `cruzetad` process is running on will
be able to:
...
unprivileged) on the hardware your `cruzetad` process runs on until these
