...
$0 [ --enable-lcov || --disable-tests ] [ --disable-mining ] [ --disable-libs ] [ MAKEARGS... ]
  Build CruZeta and most of its transitive dependencies from
  source. MAKEARGS are applied to both dependencies and CruZeta itself.

  If --enable-lcov is passed, CruZeta is configured to add coverage
  instrumentation, thus enabling "make cov" to work.
  If --disable-tests is passed instead, the CruZeta tests are not built.

  If --disable-mining is passed, CruZeta is configured to not build any mining
  code. It must be passed after the test arguments, if present.

  If --disable-libs is passed, CruZeta is configured to not build any libraries like
  'libbitcoinconsensus'.
EOF
    exit 0
fi
