...
def main(args=sys.argv[1:]):
    """
    Perform the final CruZeta release process up to the git tag.
    """
...
def update_debian_changelog(release):
    os.environ['DEBEMAIL'] = 'developers@cruzeta.info'
    os.environ['DEBFULLNAME'] = 'The CruZeta Developers'
    sh_log(
        'debchange',
...
def patch_gitian_linux_yml(release, releaseprev):
    ...
    with PathPatcher(path) as (inf, outf):
        ...
        assert secondline == 'name: "cruzeta-{}"\n'.format(
            releaseprev.novtext
        ), repr(secondline)

        outf.write('name: "cruzeta-{}"\n'.format(release.novtext))
        outf.write(inf.read())
...
def initialize_logging():
    logname = './cruzeta-make-release.log'
    ...
    logging.info('cruzeta make-release.py debug log: %r', logname)
