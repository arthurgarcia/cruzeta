...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "cruzeta.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "cruzetad.pid"));
#endif
...
)
{
    ...
    if (!(
        ...
        // Create the 'cruzeta-params' directory or the 'CruZetaParams' folder
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Initialize CruZeta circuit parameters
    if (!(ZC_LoadParams(chainparams)))
        return InitError(_("Error downloading or verifying CruZeta network parameters"));
    ...
    if (GetBoolArg("-minetolocalwallet", false)) {
        return InitError(_("CruZeta was not built with wallet support. Set -minetolocalwallet=0 to use -mineraddress, or rebuild CruZeta with wallet support."));
    }
    if (GetArg("-mineraddress", "").empty() && GetBoolArg("-gen", false)) {
        return InitError(_("CruZeta was not built with wallet support. Set -mineraddress, or rebuild CruZeta with wallet support."));
    }
 #endif // !ENABLE_WALLET
