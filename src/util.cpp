...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CruZeta
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CruZeta
    // Mac: ~/Library/Application Support/CruZeta
    // Unix: ~/.cruzeta
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CruZeta";
#else
    fs::path pathRet;
...
{
    // Copied from GetDefaultDataDir and adapter for cruzeta params.

    namespace fs = boost::filesystem;
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CruZetaParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CruZetaParams
    // Mac: ~/Library/Application Support/CruZetaParams
    // Unix: ~/.cruzeta-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CruZetaParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "CruZetaParams";
#else
    // Unix
    return pathRet / ".cruzeta-params";
#endif
#endif
}
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "cruzeta.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No cruzeta.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override cruzeta.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "cruzetad.pid"));
...
std::string LicenseInfo()
{
    return "\n" +
           ...
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The CruZeta Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
