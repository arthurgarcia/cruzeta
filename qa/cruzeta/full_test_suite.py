#!/usr/bin/env python2
#
# Execute all of the automated tests related to CruZeta.
#
...
def test_rpath_runpath(filename):
    output = subprocess.check_output(
        [repofile('qa/cruzeta/checksec.sh'), '--file', repofile(filename)]
    )
...
def test_fortify_source(filename):
    proc = subprocess.Popen(
        [repofile('qa/cruzeta/checksec.sh'), '--fortify-file', repofile(filename)],
...
def check_security_hardening():
    ...
    ret &= test_rpath_runpath('src/cruzetad')
    ret &= test_rpath_runpath('src/cruzeta-cli')
    ret &= test_rpath_runpath('src/cruzeta-gtest')
    ret &= test_rpath_runpath('src/cruzeta-tx')
    ret &= test_rpath_runpath('src/test/test_bitcoin')

    # NOTE: checksec.sh does not reliably determine whether FORTIFY_SOURCE
    # is enabled for the entire binary. See issue #915.
    ret &= test_fortify_source('src/cruzetad')
    ret &= test_fortify_source('src/cruzeta-cli')
    ret &= test_fortify_source('src/cruzeta-gtest')
    ret &= test_fortify_source('src/cruzeta-tx')
...
STAGE_COMMANDS = {
    ...
    'gtest': [repofile('src/cruzeta-gtest')],
