...
def initialize_datadir(dirname, n):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    with open(os.path.join(datadir, "cruzeta.conf"), 'w') as f:
        f.write("regtest=1\n")
...
def initialize_chain(test_dir):
    ...
    4 wallets.
    cruzetad and cruzeta-cli must be in search path.
    """

    if not os.path.isdir(os.path.join("cache", "node0")):
        devnull = open("/dev/null", "w+")
        # Create cache directories, run cruzetads:
        for i in range(4):
            datadir=initialize_datadir("cache", i)
            args = [ os.getenv("LITECOINZD", "cruzetad"), "-keypool=1", "-datadir="+datadir, "-discover=0" ]
            if i > 0:
                args.append("-connect=127.0.0.1:"+str(p2p_port(0)))
            cruzetad_processes[i] = subprocess.Popen(args)
            if os.getenv("PYTHON_DEBUG", ""):
                print "initialize_chain: cruzetad started, calling cruzeta-cli -rpcwait getblockcount"
            subprocess.check_call([ os.getenv("LITECOINZCLI", "cruzeta-cli"), "-datadir="+datadir,
                                    "-rpcwait", "getblockcount"], stdout=devnull)
            if os.getenv("PYTHON_DEBUG", ""):
                print "initialize_chain: cruzeta-cli -rpcwait getblockcount completed"
        devnull.close()
        rpcs = []
        for i in range(4):
            try:
    ...
    for i in range(4):
        ...
        initialize_datadir(test_dir, i) # Overwrite port/rpcport in cruzeta.conf
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None):
    """
    Start a cruzetad and return RPC connection to it
    """
    datadir = os.path.join(dirname, "node"+str(i))
    if binary is None:
        binary = os.getenv("LITECOINZD", "cruzetad")
    args = [ binary, "-datadir="+datadir, "-keypool=1", "-discover=0", "-rest" ]
    if extra_args is not None: args.extend(extra_args)
    cruzetad_processes[i] = subprocess.Popen(args)
    devnull = open("/dev/null", "w+")
    if os.getenv("PYTHON_DEBUG", ""):
        print "start_node: cruzetad started, calling cruzeta-cli -rpcwait getblockcount"
    subprocess.check_call([ os.getenv("LITECOINZCLI", "cruzeta-cli"), "-datadir="+datadir] +
                          _rpchost_to_args(rpchost)  +
                          ["-rpcwait", "getblockcount"], stdout=devnull)
    if os.getenv("PYTHON_DEBUG", ""):
        print "start_node: calling cruzeta-cli -rpcwait getblockcount returned"
    devnull.close()
...
def start_nodes(num_nodes, dirname, extra_args=None, rpchost=None, binary=None):
    """
    Start multiple cruzetads, return RPC connections to them
    """
...
def stop_node(node, i):
    node.stop()
    cruzetad_processes[i].wait()
    del cruzetad_processes[i]
...
def wait_cruzetads():
    # Wait for all cruzetads to cleanly exit
    for cruzetad in cruzetad_processes.values():
        cruzetad.wait()
    cruzetad_processes.clear()
